                ...
                <div>
                    {
                        popularBlog.slice(0, 5).map(blog => <div className='my-5 border-b-2 border-spacing-2 px-4' key={blog.id}>
                            ...
                            <Link to={`http://localhost:5173/blogs/${blog.id}`}  className='inline-flex items-center pb-2 text-base hover:text-orange-500'>Read now <FaArrowRight className='mt-1 ml-2'/></Link>
                        </div>)
                    }
                </div>
            </div>
